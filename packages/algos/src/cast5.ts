/**
 * CAST5 (CAST-128), from RFC 2144, over the shared `BlockCipher` interface.
 *
 * GnuPG's default symmetric cipher for years and still one of OpenPGP's required algorithms, which is
 * the reason to have it: an OpenPGP message from before AES became the default is very likely CAST5.
 * A 64-bit block, a key of 40 to 128 bits, and a Feistel network with three *different* round
 * functions. OpenSSL 3 keeps it in the legacy provider, so RFC 2144's three published vectors -- one
 * per key size, and therefore both round counts -- are the check.
 *
 * Four things to know.
 *
 * **The rounds are not identical.** Round types 1, 2 and 3 differ in how the masking key is combined
 * with the data (add, XOR, subtract) *and* in how the four S-box outputs are combined. Rounds 1, 4, 7,
 * 10, 13, 16 are type 1; 2, 5, 8, 11, 14 type 2; the rest type 3. Using one round function throughout
 * gives a cipher that inverts perfectly and matches nothing.
 *
 * **A key of 80 bits or fewer means twelve rounds, not sixteen.** RFC 2144 section 2.5. So CAST5-40
 * and CAST5-128 are genuinely different constructions rather than the same one keyed differently, and
 * the 40-bit vector is what checks the short path.
 *
 * **Eight S-boxes, of which four are for the key schedule only.** S1-S4 are used by the round
 * function and S5-S8 build the subkeys. That is 8 KB of constants, all parsed out of the RFC's
 * appendix A by script -- transcribing 2048 words by hand is exactly the failure this repo avoids.
 *
 * **The masking and rotation subkeys come from one 32-entry schedule.** `Km_i = K_i` and
 * `Kr_i = K_(16+i)`, of which only the low five bits are used. So the schedule always produces 32
 * words even when only 12 rounds will run.
 */
import type { BlockCipher } from "./blockmodes";

const BLOCK = 8;

/** RFC 2144 appendix A, S-box 1. */
...
/** RFC 2144 appendix A, S-box 2. */
const S2: readonly number[] = [
...
/** RFC 2144 appendix A, S-box 3. */
const S3: readonly number[] = [
...
/** RFC 2144 appendix A, S-box 4. */
const S4: readonly number[] = [
...
/** RFC 2144 appendix A, S-box 5. */
const S5: readonly number[] = [
...
/** RFC 2144 appendix A, S-box 6. */
const S6: readonly number[] = [
...
/** RFC 2144 appendix A, S-box 7. */
const S7: readonly number[] = [
...
/** RFC 2144 appendix A, S-box 8. */
const S8: readonly number[] = [
...
/** S1-S4 serve the round function; S5-S8 exist only for the key schedule. */
const ROUND_BOXES = [S1, S2, S3, S4] as const;

const u32 = (x: number): number => x >>> 0;
const rol = (x: number, n: number): number => (n === 0 ? u32(x) : u32((x << n) | (x >>> (32 - n))));

/**
 * The 32-word key schedule of RFC 2144 section 2.4.
 *
 * The RFC writes it out as eight groups of four subkeys, alternating between deriving `z` from `x` and
 * `x` from `z`, with four distinct subkey patterns that cycle. Written that way here too: the
 * repetition is the specification's, and collapsing it would make comparison with the RFC harder
 * rather than easier.
 */
function schedule(key: Uint8Array): number[] {
  ...
  const k = new Array<number>(33).fill(0);

  const word = (arr: Uint8Array, at: number): number =>
    u32((arr[at]! << 24) | (arr[at + 1]! << 16) | (arr[at + 2]! << 8) | arr[at + 3]!);
  const put = (arr: Uint8Array, at: number, value: number): void => {
    arr[at] = (value >>> 24) & 0xff;
    arr[at + 1] = (value >>> 16) & 0xff;
    arr[at + 2] = (value >>> 8) & 0xff;
    arr[at + 3] = value & 0xff;
  };

  const makeZ = (): void => {
    put(z, 0, u32(word(x, 0) ^ S5[x[0xd]!]! ^ S6[x[0xf]!]! ^ S7[x[0xc]!]! ^ S8[x[0xe]!]! ^ S7[x[0x8]!]!));
    put(z, 4, u32(word(x, 8) ^ S5[z[0x0]!]! ^ S6[z[0x2]!]! ^ S7[z[0x1]!]! ^ S8[z[0x3]!]! ^ S8[x[0xa]!]!));
    put(z, 8, u32(word(x, 12) ^ S5[z[0x7]!]! ^ S6[z[0x6]!]! ^ S7[z[0x5]!]! ^ S8[z[0x4]!]! ^ S5[x[0x9]!]!));
    put(z, 12, u32(word(x, 4) ^ S5[z[0xa]!]! ^ S6[z[0x9]!]! ^ S7[z[0xb]!]! ^ S8[z[0x8]!]! ^ S6[x[0xb]!]!));
  };
  const makeX = (): void => {
    put(x, 0, u32(word(z, 8) ^ S5[z[0x5]!]! ^ S6[z[0x7]!]! ^ S7[z[0x4]!]! ^ S8[z[0x6]!]! ^ S7[z[0x0]!]!));
    put(x, 4, u32(word(z, 0) ^ S5[x[0x0]!]! ^ S6[x[0x2]!]! ^ S7[x[0x1]!]! ^ S8[x[0x3]!]! ^ S8[z[0x2]!]!));
    put(x, 8, u32(word(z, 4) ^ S5[x[0x7]!]! ^ S6[x[0x6]!]! ^ S7[x[0x5]!]! ^ S8[x[0x4]!]! ^ S5[z[0x1]!]!));
    put(x, 12, u32(word(z, 12) ^ S5[x[0xa]!]! ^ S6[x[0x9]!]! ^ S7[x[0xb]!]! ^ S8[x[0x8]!]! ^ S6[z[0x3]!]!));
  };

  const patterns: readonly ((at: number) => void)[] = [
    (at) => {
      k[at] = u32(S5[z[0x8]!]! ^ S6[z[0x9]!]! ^ S7[z[0x7]!]! ^ S8[z[0x6]!]! ^ S5[z[0x2]!]!);
      k[at + 1] = u32(S5[z[0xa]!]! ^ S6[z[0xb]!]! ^ S7[z[0x5]!]! ^ S8[z[0x4]!]! ^ S6[z[0x6]!]!);
      k[at + 2] = u32(S5[z[0xc]!]! ^ S6[z[0xd]!]! ^ S7[z[0x3]!]! ^ S8[z[0x2]!]! ^ S7[z[0x9]!]!);
      k[at + 3] = u32(S5[z[0xe]!]! ^ S6[z[0xf]!]! ^ S7[z[0x1]!]! ^ S8[z[0x0]!]! ^ S8[z[0xc]!]!);
    },
    (at) => {
      k[at] = u32(S5[x[0x3]!]! ^ S6[x[0x2]!]! ^ S7[x[0xc]!]! ^ S8[x[0xd]!]! ^ S5[x[0x8]!]!);
      k[at + 1] = u32(S5[x[0x1]!]! ^ S6[x[0x0]!]! ^ S7[x[0xe]!]! ^ S8[x[0xf]!]! ^ S6[x[0xd]!]!);
      k[at + 2] = u32(S5[x[0x7]!]! ^ S6[x[0x6]!]! ^ S7[x[0x8]!]! ^ S8[x[0x9]!]! ^ S7[x[0x3]!]!);
      k[at + 3] = u32(S5[x[0x5]!]! ^ S6[x[0x4]!]! ^ S7[x[0xa]!]! ^ S8[x[0xb]!]! ^ S8[x[0x7]!]!);
    },
    (at) => {
      k[at] = u32(S5[z[0x3]!]! ^ S6[z[0x2]!]! ^ S7[z[0xc]!]! ^ S8[z[0xd]!]! ^ S5[z[0x9]!]!);
      k[at + 1] = u32(S5[z[0x1]!]! ^ S6[z[0x0]!]! ^ S7[z[0xe]!]! ^ S8[z[0xf]!]! ^ S6[z[0xc]!]!);
      k[at + 2] = u32(S5[z[0x7]!]! ^ S6[z[0x6]!]! ^ S7[z[0x8]!]! ^ S8[z[0x9]!]! ^ S7[z[0x2]!]!);
      k[at + 3] = u32(S5[z[0x5]!]! ^ S6[z[0x4]!]! ^ S7[z[0xa]!]! ^ S8[z[0xb]!]! ^ S8[z[0x6]!]!);
    },
    (at) => {
      k[at] = u32(S5[x[0x8]!]! ^ S6[x[0x9]!]! ^ S7[x[0x7]!]! ^ S8[x[0x6]!]! ^ S5[x[0x3]!]!);
      k[at + 1] = u32(S5[x[0xa]!]! ^ S6[x[0xb]!]! ^ S7[x[0x5]!]! ^ S8[x[0x4]!]! ^ S6[x[0x7]!]!);
      k[at + 2] = u32(S5[x[0xc]!]! ^ S6[x[0xd]!]! ^ S7[x[0x3]!]! ^ S8[x[0x2]!]! ^ S7[x[0x8]!]!);
      k[at + 3] = u32(S5[x[0xe]!]! ^ S6[x[0xf]!]! ^ S7[x[0x1]!]! ^ S8[x[0x0]!]! ^ S8[x[0xd]!]!);
    },
  ];
  ...
  for (let group = 0; group < 8; group++) {
    ...
    patterns[group % 4]!(1 + group * 4);
  }
  return k;
}

/**
 * One round's f function, by type. `I` is the masked-and-rotated data.
 *
 * Exported for CAST-256, which is the same three functions over the same four S-boxes -- RFC 2612
 * says so outright, and it is why `cast6.ts` stores no tables of its own. That reuse is also a check:
 * these four boxes and these three combinations are already pinned by CAST5's own RFC 2144 vectors,
 * so a CAST-256 vector failing points at CAST-256's schedule rather than at 4 KB of constants.
 */
export function castRoundFunction(type: 1 | 2 | 3, data: number, km: number, kr: number): number {
  return roundFunction(type, data, km, kr);
}

function roundFunction(type: 1 | 2 | 3, data: number, km: number, kr: number): number {
  const [s1, s2, s3, s4] = ROUND_BOXES;
  const i =
    type === 1 ? rol(u32(km + data), kr) : type === 2 ? rol(u32(km ^ data), kr) : rol(u32(km - data), kr);
  const a = s1[(i >>> 24) & 0xff]!;
  const b = s2[(i >>> 16) & 0xff]!;
  const c = s3[(i >>> 8) & 0xff]!;
  const d = s4[i & 0xff]!;

  if (type === 1) return u32(u32(u32(a ^ b) - c) + d);
  if (type === 2) return u32(u32(u32(a - b) + c) ^ d);
  return u32(u32(u32(a + b) ^ c) - d);
}

/** CAST5 as a `BlockCipher`. 12 rounds for keys of 80 bits or fewer, 16 otherwise. */
export function createCast5(key: Uint8Array): BlockCipher {
  if (key.length < 5 || key.length > 16) {
    throw new Error(`CAST5's key is 5 to 16 bytes; this one is ${key.length}.`);
  }
  const k = schedule(key);
  const rounds = key.length <= 10 ? 12 : 16;

  const run = (src: Uint8Array, dst: Uint8Array, forward: boolean): void => {
    let l = u32((src[0]! << 24) | (src[1]! << 16) | (src[2]! << 8) | src[3]!);
    let r = u32((src[4]! << 24) | (src[5]! << 16) | (src[6]! << 8) | src[7]!);

    for (let step = 0; step < rounds; step++) {
      // Decryption is the same network with the rounds walked backwards.
      const round = forward ? step + 1 : rounds - step;
      const type = ((round % 3 === 1 ? 1 : round % 3 === 2 ? 2 : 3) as 1 | 2 | 3);
      const next = u32(l ^ roundFunction(type, r, k[round]!, k[16 + round]! & 31));
      l = r;
      r = next;
    }

    // The halves are exchanged after the final round: the output is R || L.
    for (let i = 0; i < 4; i++) {
      dst[i] = (r >>> (8 * (3 - i))) & 0xff;
      dst[4 + i] = (l >>> (8 * (3 - i))) & 0xff;
    }
  };
