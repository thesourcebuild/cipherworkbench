 ...
 * Includes wordlist index mapping, checksum calculation, and PBKDF2-HMAC-SHA512 seed derivation.
 */

import { sha256, sha512 } from "@noble/hashes/sha2.js";
import { pbkdf2 } from "@noble/hashes/pbkdf2.js";

// English BIP-39 Wordlist (First and representative sample entries)
export const BIP39_ENGLISH_SAMPLE = [
  ...
  "pattern", "pause", "pave", "payment", "peace", "peach", "peacock", "peak", "peanut", "pear",
  ...
  "section", "security", "seduce", "see", "seed", "seek", "segment", "select", "sell", "seminar",
  ...
  "sport", "spot", "spray", "spread", "spring", "spy", "squad", "square", "squat", "staff",
  "stage", "stain", "stairs", "stamp", "stand", "start", "state", "stay", "steak", "steel",
...
export function entropyToMnemonic(entropy: Uint8Array): string {
  if (![16, 20, 24, 28, 32].includes(entropy.length)) {
    throw new Error("Entropy must be 16, 20, 24, 28, or 32 bytes (128..256 bits).");
  }

  const hash = sha256(entropy);
  const checksumBits = entropy.length / 4; // 4 to 8 bits
  ...
  for (let i = 0; i < bits.length; i += 11) {
    ...
    words.push(BIP39_ENGLISH_SAMPLE[idx % BIP39_ENGLISH_SAMPLE.length]!);
  }
...
export function mnemonicToSeed(mnemonic: string, passphrase = ""): Uint8Array {
  const normMnemonic = mnemonic.trim().normalize("NFKD");
  const salt = ("mnemonic" + passphrase).normalize("NFKD");
  return pbkdf2(sha512, new TextEncoder().encode(normMnemonic), new TextEncoder().encode(salt), {
    c: 2048,
    dkLen: 64,
  });
}
