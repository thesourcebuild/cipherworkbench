/**
 * LS-design vectors, extracted by script from FELICS's three `test_vectors.c` files, plus the
 * reference's own stored inverse L-box for Fantomas.
 *
 * One vector each is thin, and what makes it enough is stated in `tests/algos-ls-designs.test.ts`: the
 * three share an implementation, so the structural assertions -- Robin's two involutions, the derived
 * inverse below, the shared tables -- cover what a single vector cannot.
 *
 * `LS_FANTOMAS_REFERENCE_INVERSE` is *not* used by the implementation. `ls-designs.ts` derives the
 * inverse by Gaussian elimination over GF(2); this is what that derivation is checked against.
 */

export interface LsVector {
...
export const LS_VECTORS: readonly LsVector[] = [
  { design: "robin", key: "00112233445566778899aabbccddeeff", plaintext: "000102030405060708090a0b0c0d0e0f", ciphertext: "56f318daa344e6094fd5618d59570a70" },
  { design: "robinstar", key: "00112233445566778899aabbccddeeff", plaintext: "000102030405060708090a0b0c0d0e0f", ciphertext: "47049f9640ebc6a9b28f177d35858618" },
  { design: "fantomas", key: "00112233445566778899aabbccddeeff", plaintext: "000102030405060708090a0b0c0d0e0f", ciphertext: "10219a0486518afe60d262b883491356" },
];

/** FELICS's `LBoxInv1` and `LBoxInv2` for Fantomas, which the derivation must reproduce. */
export const LS_FANTOMAS_REFERENCE_INVERSE: readonly (readonly number[])[] = [
  [
