...
CLIENT_CERT = os.path.join(DIR, "client.crt")
...
def test_authenticated(tls_version="auto"):
    print(f"\n--- Test 1: Mutual TLS Handshake (TLS: {tls_version}) ---")
    ...
    try:
        ...
        print("  ✓ PASS: Mutual TLS Connection Successful!")
...
def main():
    if not os.path.exists(CA_FILE) or not os.path.exists(CLIENT_CERT) or not os.path.exists(CLIENT_KEY):
        print(f"Error: Certificate files not found in {DIR}. Run 'npx tsx run.ts' first.")
        sys.exit(1)

    parser = argparse.ArgumentParser(description="Python 2-Tier mTLS Client")
    parser.add_argument("--tls-version", choices=["auto", "1.1", "1.2", "1.3", "all"], default="auto", help="TLS version to request")
    args = parser.parse_args()

    print("=================================================")
    print("   PYTHON CLIENT: 2-TIER mTLS TEST               ")
    print("=================================================")
